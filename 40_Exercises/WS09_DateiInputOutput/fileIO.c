...
{	
	...
	fptrOutput=fopen("./studentArray.csv","w");
	for (int i=0;i<studentCount;i++)
	{
		fprintf(fptrOutput,"%s,%s,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f\n", studentArray[i].firstName, studentArray[i].lastName,studentArray[i].grades[1],studentArray[i].grades[2],studentArray[i].grades[3],studentArray[i].grades[4],studentArray[i].grades[5],studentArray[i].grades[6],studentArray[i].average);
	...
	*/
	...
	//read from csv
	struct student studentArray[studentCount+4];
	fptrInput=fopen("./studentArray.csv","w");
